...
| **`flow`** | ワークフローコマンド `go`/`cm`/`e2e`/`pin`/`vet` | 前の 4 つは `/flow:go`、`/flow:cm`、`/flow:e2e`、`/flow:pin` で手動呼び出し。`vet` は `cm` がコミット前に自動で呼ぶ |
...
`flow` plugin は手動トリガーのワークフローコマンドを 4 つ含みます。`disable-model-invocation: true` を設定しており、`/` を入力したときだけ呼び出され、Claude が自動トリガーすることはなく、description も context に入りません——`/` メニューのラベルでしかありません。5 つ目の `vet` はコミット前のチェッカーで、Claude が自分で呼ぶことができ、`cm` は毎回のコミット前に呼びます。
...
| `vet` | コミット前のチェック：`context: fork` で会話が見えないサブエージェントとして動き、diff とメッセージ草稿だけを受け取って一文ずつ照合する。diff に対応しない文、リポジトリで見つからない番号参照、経緯を語る・長すぎるコメント、中国語以外の文を指摘する。報告のみでファイルは変更しない。`cm` が毎コミット前に呼ぶほか、作業途中にコメントだけ確認したいときは直接呼べる | `/flow:vet` |

> これらは作者個人のワークフローコマンドです。`go` は誰でも使える一般的な規律です。`cm` のコミットメッセージ書式は作者のグローバルな `~/.claude/CLAUDE.md` のルールを参照しているので、自分のコミット規約に差し替えてください。`e2e` は計画・生成・修復に公式 `playwright-cli` skill（コンテナがプロジェクトの `e2e/` 内に導入）を使います。実行環境の構築は `plugins/flow/skills/e2e/references/runner.md` を参照。`pin` はこのコンテナと `references/` の原因特定・タイミングの資料を `e2e` と共用します。
